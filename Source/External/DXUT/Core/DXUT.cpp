                          ...
                          bool bForceRecreate, bool bClipWindowToSingleAdapter )
{
	...
	DXUTDeviceSettings* _pNewDeviceSettings = pNewDeviceSettings;
        ...
        DXUTCanDeviceBeReset( pOldDeviceSettings, pNewDeviceSettings, pd3d9DeviceFromApp, pd3d10DeviceFromApp ) )
    {
        ...
        if( FAILED( hr ) )
        {
            ...
            else // DXUTERR_RESETTINGDEVICE
            {
				...
				delete pNewDeviceSettings;
                if( FAILED( DXUTChangeDevice( _pNewDeviceSettings, pd3d9DeviceFromApp, pd3d10DeviceFromApp, true,
...
void DXUTDisplayErrorMessage( HRESULT hr )
{
    ...
    switch( hr )
    {
        ...
        case DXUTERR_MEDIANOTFOUND:
            nExitCode = 4; StringCchCopy( strBuffer, ARRAYSIZE(strBuffer), L"Could not find required media." ); break;
        ...
        case DXUTERR_CREATINGDEVICE:
            nExitCode = 6; StringCchCopy( strBuffer, ARRAYSIZE(strBuffer), L"Failed creating the Direct3D device." );
            break;
        case DXUTERR_RESETTINGDEVICE:
            nExitCode = 7; StringCchCopy( strBuffer, ARRAYSIZE(strBuffer), L"Failed resetting the Direct3D device." );
            break;
        ...
        case DXUTERR_DEVICEREMOVED:
            nExitCode = 11; StringCchCopy( strBuffer, ARRAYSIZE(strBuffer), L"The Direct3D device was removed." );
            break;
        default:
...
void DXUTUpdateStaticFrameStats()
{
    ...
    if( DXUTIsD3D9( pDeviceSettings ) )
    {
        ...
        switch( pPP->MultiSampleType )
        {
            case D3DMULTISAMPLE_NONMASKABLE:
                StringCchCopy( strMultiSample, 100, L" (Nonmaskable Multisample)" ); break;
            case D3DMULTISAMPLE_NONE:
                StringCchCopy( strMultiSample, 100, L"" ); break;
            default:
                StringCchPrintf( strMultiSample, 100, L" (%dx Multisample)", pPP->MultiSampleType ); break;
        }
    ...
    else
    {
        ...
        WCHAR strMultiSample[100];
        StringCchPrintf( strMultiSample, 100, L" (MS%u, Q%u)", pDeviceSettings->d3d10.sd.SampleDesc.Count,
